...
const ImgPremios_a = () => {
  ...
  // Función para obtener las imágenes de premios desde el servidor
  const fetchImages = async () => {
    try {
      ...
      console.error("Error fetching premios images:", error);
      Swal.fire({
        icon: "error",
        title: "Error",
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      const response = await fetch(`${config.api}uploadPremiosImages.php`, {
        method: "POST",
        credentials: "include",
        body: formData,
      });
  ...
  const handleDeleteImage = (image) => {
    MySwal.fire({
      title: "¿Estás seguro?",
      text: "¿Quieres eliminar esta imagen de los premios?",
      icon: "question",
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(
            `${config.api}deletePremiosImage.php?image=${image.image}&id=${image.id}`,
            {
              credentials: "include",
              method: "get",
            }
        ...
        <div className="inner_admin">
          <div className="left">
            <div className="inner_grid">
              <h2>Actualizar imagenes de los premios</h2>
            ...
            <div className="inner_grid banner">
              {images.map((image, index) => (
                <div key={index} className="banner-image">
                  <img
                    src={`${config.public_awards}${image.image}`}
                    alt={`Banner ${image}`}
                  />
                  <button
